...
from utils import get_nodes_edge, nodes_edge_to_tensor,get_reward, get_pre_compile_passmanager
...
class qiskit_env:
    def __init__(self, *args, **kwargs):
        ...
        self.model = 'train'
        self.circuits_qasm_path = "../circuits/MQTBench_2_30_train/"
        ...
        self.action_dict = {0 : [Collect2qBlocks(), opt_pass_dict[0], UnitarySynthesis(basis_gates=self.basis_gates)],
                            ...
                            2 : [opt_pass_dict[2]],
                            3 : [opt_pass_dict[3]]}
        self.pre_passmanager = get_pre_compile_passmanager()
        qasm_files = [self.circuits_qasm_path + file for file in os.listdir(self.circuits_qasm_path)]
        for qasm_file in tqdm(qasm_files):
            current_circ = QuantumCircuit.from_qasm_file(qasm_file)
            init_circuit = self.pre_passmanager.run(current_circ)
            self.init_circuit_states.append(init_circuit)

    def reset(self):
        self.current_circ = None
        self.compiling_passmanager = PassManager()
        self.model = 'train'
        self.current_circ = self.get_random_init_state()
    ...
    def get_random_init_state(self):
        random_init_state = random.choice(self.init_circuit_states)
