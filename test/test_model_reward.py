...
device_cuda = torch.device("cpu" if torch.cuda.is_available() else "cpu")
...
def get_all_results():
    ...
    file_dir = "../new_circuits/MQTBench_2_30_train/"
    ...
    def process_file(qc_file):
        ...
        init_qc = QuantumCircuit.from_qasm_file(qc_file)
        
        qc = env.pre_passmanager.run(init_qc)
        env.current_circ = qc
    ...
    for i in tqdm(chunks):
        ...
        try:
            if flag == 0:
                q_network = torch.load('../train/e_greedy/model/node_gate_num/GAT_learn_rate_000001_update_30000_net_256_relu_memory_memory_batch_128_e_greedy_no_features/_' + str(i) + '/policy_model.pth', map_location=torch.device('cpu'),weights_only=False)
            elif flag == 1:
                q_network = torch.load('../train/e_greedy/model/node_gate_num/GAT_learn_rate_000001_update_30000_net_256_batch_128_relu_DoubleDQN_node_gate_num_distribution/_' + str(i) + '/policy_model.pth', map_location=torch.device('cpu'),weights_only=False)
            elif flag == 2:
                q_network = torch.load('../train/e_greedy/model/node_gate_num/GAT_learn_rate_000001_30000_net_256_relu_memory_30000_batch_128_e_greedy_O3_preprocessing/_' + str(i) + '/policy_model.pth', map_location=torch.device('cpu'),weights_only=False)
            q_network.eval()                                                                            
...
if __name__ == '__main__':
    if flag == 0:
        ...
        param_set= 'GAT_learn_rate_000001_30000_net_256_relu_memory_30000_batch_128_e_greedy_O3_preprocessing'
