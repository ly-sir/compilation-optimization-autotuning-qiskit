...
def get_data(train=False, test=False, third=False, big=False):
    ...
    ##################################################################################################################################
    if big:
        a_result = "../data/opt_rate/parallelism/"+ param_set +"/_"+ str(n) +"/model_all_MQT_2_30_circuits_result_31_40.txt"
        b_result = "../data/opt_rate/parallelism/"+ param_set +"/_"+ str(n) +"/qiskit_all_MQT_2_30_circuits_result_31_40.txt"
    if test:
        a_result = "../data/opt_rate/parallelism/"+ param_set +"/_"+ str(n) +"/model_all_MQT_2_30_test_circuits_result.txt"
        b_result = "../data/opt_rate/parallelism/"+ param_set +"/_"+ str(n) +"/qiskit_all_MQT_2_30_test_circuits_result.txt"
        
    if train:
        a_result = "../data/opt_rate/parallelism/"+ param_set +"/_"+ str(n) +"/model_all_MQT_2_30_train_circuits_result.txt"
        b_result = "../data/opt_rate/parallelism/"+ param_set +"/_"+ str(n) +"/qiskit_all_MQT_2_30_train_circuits_result.txt"

    if third:
        a_result = "../data/opt_rate/parallelism/"+ param_set +"/_"+ str(n) +"/model_test_circuits_result_third.txt"
        b_result = "../data/opt_rate/parallelism/"+ param_set +"/_"+ str(n) +"/qiskit_test_circuits_result_third.txt"
...
def get_increase_rate_data(algorithm_names, before_a, after_a, before_b, after_b):
    ...
    for key in performance_dict_a.keys():
        if key in performance_dict_b:
            ...
            if dict_count[key] != 0:
                improvement = (a_value - b_value)/ dict_count[key]
            else:
                print(key)
                improvement = 0
...
def plot_4x2(x_values_list, y_values_list, texts, left_colors, right_colors, rate_texts, bar_labels_list, bar_values_list):
    ...
    # plt.suptitle("Overall Title of the Figure", fontsize=14, y=0.98)  # Adjusted suptitle position and fontsize
    plt.show()
...
def new_count_rate():
    ...
    change_num_a_vs_b = [(after - after1)for after, after1 in zip(after_a, after_b)]
    print(f"###########################change_num_a_vs_b parallelism :{sum(change_num_a_vs_b)}")
    # 计算每种情况的变化率
    change_rates_a_vs_b_train = [(after - after1) / after1 for before, after, after1 in zip(before_b_train, after_a_train, after_b_train)]
    change_rates_a_vs_b_test = [(after - after1) / after1 for before, after, after1 in zip(before_b_test, after_a_test, after_b_test)]
    change_rates_a_vs_b_test_31_40 = [(after - after1) / after1 for before, after, after1 in zip(before_b_test_31_40, after_a_test_31_40, after_b_test_31_40)]
    change_rates_a_vs_b_test_third = [(after - after1) / after1 for before, after, after1 in zip(before_b_test_third, after_a_test_third, after_b_test_third)]
    ...
    # 将数据传递给 plot_4x2 函数
    # plot_4x2(
    #     [list(range(len(change_rates_a_vs_b_train))), list(range(len(change_rates_a_vs_b_test))), list(range(len(change_rates_a_vs_b_test_31_40)))],
    #     [change_rates_a_vs_b_train, change_rates_a_vs_b_test, change_rates_a_vs_b_test_31_40],
    #     ["Train Set (2-30 bit)", "Test Set 1 (2-30 bit)", "Test Set 2 (31-40 bit)", "Test Set 3 (other algorithms)"],
    #     ['blue','purple','green','brown'],
    #     ['cornflowerblue', 'lightsalmon', 'lightsalmon', 'lightsalmon'],
    #     [rate(algorithm_names_train, after_a_train, after_b_train), rate(algorithm_names_test, after_a_test, after_b_test), rate(algorithm_names_test_31_40, after_a_test_31_40, after_b_test_31_40)],
    #     [labels_train, labels_test, labels_test_31_40],
    #     [values_train, values_test, values_test_31_40]
    # )
...
def depth_():
    ...
    # 计算每种情况的变化率
    change_rates_a_vs_b = [(after - after1) / after1 for after, after1 in zip(after_a, after_b)]
